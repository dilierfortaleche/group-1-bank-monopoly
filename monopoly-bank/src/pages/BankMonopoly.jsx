...
export default function BalanceJugadores() {
  ...
  const handleFinallyTurn = () => {
    Swal.fire({
      icon: "error",
      title: "En banca rota",
      text: "Te acabas de declarar en Banca rota",
  ...
  return (
      <div className="p-6 mt-10 bg-transparent rounded-lg shadow-lg w-90 text-center">
        ...
        <ul className="bg-gray-200 p-3 rounded-md shadow-md w-full">
          {players.map((player) => (
            <li key={player.id} className="text-lg text-gray-800 flex justify-between p-2 border-b">
        ...
        <div className="mt-4 space-y-3">
          <button onClick={() => handleReportTransc()}
          ...
          <button onClick={() => handleFinallyTurn()}
          className="w-full p-3 text-lg rounded-md bg-red-500 text-white hover:bg-blue-600 transition shadow-md">
            Terminar turno
          </button>
